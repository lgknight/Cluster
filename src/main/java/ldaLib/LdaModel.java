...
public class LdaModel {
	...
	/**
	 * 获取文档聚类结果[docId,topicId,topicVoc](文档Id, 话题Id， 话题代表词汇)
	 * */
	public ArrayList<LDAResult> doc2topic(){
		LinkedList<LDAResult> ldaResults = new LinkedList<LDAResult>();
		for(int i = 0; i < M; i++) {
			double topicFre = theta[i][0];
			int topicId = 0;
			for(int j = 1; j < K; j++){
				if(topicFre < theta[i][j]){
					topicFre = theta[i][j];
					topicId = j;
				}
			}
			LDAResult ldaResult = new LDAResult(docSet.docs.get(i).getDocId(), topicId, getTopicTypicalWords(topicId));
			ldaResults.add(ldaResult);
		}
		return new ArrayList<LDAResult>(ldaResults);
	}

	/**
	 * @Description： 一个话题下前n个关键词
	 * */
	private String getTopicTypicalWords(int topicId){
		StringBuilder keyWords = new StringBuilder();
		List<Integer> tWordsIndexArray = new ArrayList<Integer>();
		for(int j = 0; j < V; j++){
			tWordsIndexArray.add(new Integer(j));
		}
		Collections.sort(tWordsIndexArray, new LdaModel.TwordsComparable(phi[topicId]));
		for(int t = 0; t < TypicalWordsNum; t++){
			keyWords.append(docSet.indexToTermMap.get(tWordsIndexArray.get(t)) + "\t");
		}
	...
	public LdaModel(LdaGibbsSampling.modelparameters modelparam) {
		...
		TypicalWordsNum = modelparam.ldaTypicalWordsNum;
	}
...
//				saveIteratedModel(i, docSet);
	...
	public void saveIteratedModel(int iters, Documents docSet) throws IOException {
		...
		//lda.params
		String resPath = System.getProperty("user.dir") .concat(File.separator).concat(PropertyHandler.getProperty("ldaResultsPath"));
